...
save_data = true;
save_name = 'mpc_sim_quadprog_1.mat'; path = 'sim_data/';
...
y_opt_0 = [repmat(x0,Np,1); repmat(u0,Nc,1); zeros(nx,1)];
...
fprintf('MPC simulation (quadprog)...\n');
for k=1:1:N_sim
	...
	options.OptimalityTolerance = 1e-06; % Infeas. tolerance

	lb_mpc = [repmat(x_lb,floor(0.5*Np),1); repmat(-inf*ones(nx,1),Np-floor(0.5*Np),1); repmat(u_lb,Nc,1); zeros(nx,1)];
	ub_mpc = [repmat(x_ub,floor(0.5*Np),1); repmat(inf*ones(nx,1),Np-floor(0.5*Np),1); repmat(u_ub,Nc,1); inf*ones(nx,1)];

	tim1 = tic;
	
	[y_opt,~,~,info,mult] = quadprog(H_mpc, c_mpc, A_ineq_mpc, b_ineq_mpc, A_eq_mpc, ...
		b_eq_mpc, lb_mpc, ub_mpc, y_opt_0, options);
	
	curr_exec_time = toc(tim1);
	curr_n_iter = info.iterations;
