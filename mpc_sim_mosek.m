...
save_data = true;
save_name = 'mpc_sim_mosek_1.mat'; path = 'sim_data/';
...
for k=1:1:N
	...
	options.MSK_DPAR_INTPNT_TOL_PFEAS = 1e-06; % Primal infeas. tolerance
	options.MSK_DPAR_INTPNT_TOL_DFEAS = 1e-06; % Dual infeas. tolerance

	prob.c = c_mpc;
	prob.a = A_eq_mpc;
	prob.blc = b_eq_mpc;
	prob.buc = b_eq_mpc;
	prob.blx = -inf(size(c_mpc));
	prob.bux = inf(size(c_mpc));
	
	prob.a = [prob.a; A_ineq_mpc];
	prob.blc = [prob.blc; -inf(size(b_ineq_mpc))];
	prob.buc = [prob.buc; b_ineq_mpc];

	tim1 = tic;
	
	sol = mskqpopt(H_mpc, prob.c, prob.a, prob.blc, prob.buc, prob.blx, prob.bux, options, 'minimize info');
	
	curr_exec_time = toc(tim1);
	curr_n_iter = sol.info.MSK_IINF_INTPNT_ITER;
	...
	mu_opt = -sol.sol.itr.xc(1:1:nx*Np);
	lambda_opt = -sol.sol.itr.xc(nx*Np+1:1:end);
