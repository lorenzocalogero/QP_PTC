...
save_data = true;
save_name = 'mpc_sim_piqp_mc_1.mat';
...
N_mc = 10; % N. of Monte Carlo simulations
...
fprintf('MPC simulation (PIQP) (Monte Carlo) (%d / %d)...\n', n, N_mc);
for k=1:1:N_sim
	...
	prob = piqp('sparse');
	prob.update_settings('eps_abs', 1e-06, 'eps_rel', 1e-06);
	prob.setup(sparse(H_mpc), c_mpc, sparse(A_eq_mpc), b_eq_mpc, sparse(A_ineq_mpc), b_ineq_mpc, [], []);
	...
	x_opt = sol.x(1:1:nx*Np);
	x_opt = reshape(x_opt, [nx,Np]);
	
	u_opt = sol.x(nx*Np+1:1:nx*Np+nu*Nc);
	u_opt = reshape(u_opt, [nu,Nc]);
	
	e_opt = sol.x(nx*Np+nu*Nc+1:1:end);
