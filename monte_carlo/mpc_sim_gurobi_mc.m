...
save_data = true;
save_name = 'mpc_sim_gurobi_mc_1.mat';
...
fprintf('MPC simulation (Gurobi) (Monte Carlo) (%d / %d)...\n', n, N_mc);
for k=1:1:N
	...
	options.FeasibilityTol = 1e-06; % Primal infeas. tol.
	options.OptimalityTol = 1e-06; % Dual infeas. tol.

	model.Q = sparse(H_mpc);
	model.obj = c_mpc;
	model.A = sparse(A_eq_mpc);
	model.rhs = b_eq_mpc;
	model.sense = '=';
	model.modelsense = 'min';
	model.QC = sparse(A_ineq_mpc);
	model.QCrhs = b_ineq_mpc;
	model.QCname = 'inequality';
	model.lb = [repmat(x_lb,floor(0.5*Np),1); repmat(-inf*ones(nx,1),Np-floor(0.5*Np),1); repmat(u_lb,Nc,1); zeros(nx,1)];
	model.ub = [repmat(x_ub,floor(0.5*Np),1); repmat(inf*ones(nx,1),Np-floor(0.5*Np),1); repmat(u_ub,Nc,1); inf*ones(nx,1)];
