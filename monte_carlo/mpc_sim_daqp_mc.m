...
save_data = true;
save_name = 'mpc_sim_daqp_mc_1.mat';
...
fprintf('MPC simulation (DAQP) (Monte Carlo) (%d / %d)...\n', n, N_mc);
for k=1:1:N_sim
	...
	prob = daqp();
	prob.setup(H_mpc, c_mpc, [A_eq_mpc; A_ineq_mpc], [b_eq_mpc; b_ineq_mpc], ...
		[b_eq_mpc; -inf*ones(length(b_ineq_mpc),1)],zeros(length([b_eq_mpc; b_ineq_mpc]),1,'int32'));
	prob.settings('primal_tol',1e-06,'dual_tol',1e-06);

	tim1 = tic;
	
	[y_opt,~,~,info] = prob.solve();
	
	curr_exec_time = toc(tim1);
	curr_n_iter = info.iter;
	...
	mu_opt = -info.lambda(1:1:nx*Np);
	lambda_opt = -info.lambda(nx*Np+1:1:end);
